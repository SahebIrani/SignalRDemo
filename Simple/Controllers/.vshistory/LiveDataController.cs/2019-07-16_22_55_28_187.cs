	...
	public class LiveDataController : Controller
	{
		...
		public async Task<IActionResult> Create(DataRealTime data, CancellationToken cancellationToken)
		{
			if (data.Id > 0)
			{
				...
				await HubContext.Clients.All.SendAsync("ReciveLiveDataFromController", data.Id, data.Title, data.Price, "edit");
			...
			else
			{
				...
				await HubContext.Clients.All.SendAsync("ReciveLiveDataFromController", dataRealTime.Id, data.Title, data.Price, "create");
